 ...
 *
 * 修改自sdk\sources\android-24\android\support\v4\widget\ViewDragHelper.java
 * 滑动方式由修改view坐标改为修改view滚动位置(scrollTo())
 */
public class SlideFinishDragHelper {
    ...
    // Angle to travel before a drag may begin
    private float mAngleSlop = (float) Math.tan(Math.PI/12);
     ...
     */
    public static SlideFinishDragHelper create(ViewGroup forParent, Callback cb) {
        return new SlideFinishDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static SlideFinishDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final SlideFinishDragHelper helper = create(forParent, cb);
     ...
     * @param forParent Parent view to monitor
     */
    private SlideFinishDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        final int startLeft = -mCapturedView.getScrollX();
        final int startTop = -mCapturedView.getScrollY();
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling(-mCapturedView.getScrollX(), -mCapturedView.getScrollY(),
                (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
                (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId),
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {
            ...
            final int dx = x - -mCapturedView.getScrollX();
            final int dy = y - -mCapturedView.getScrollY();
...
//            }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    if (pastSlop) {
                        ...
                        final int oldLeft = -toCapture.getScrollX();
                        final int targetLeft = oldLeft + (int) dx;
                        final int newLeft = mCallback.clampViewPositionHorizontal(toCapture,
                                targetLeft, (int) dx);
                        final int oldTop = -toCapture.getScrollY();
                    ...
                    if (pastSlop && !canScroll(toCapture, false, (int)dx, 0, (int)x, (int)y) && tryCaptureViewForDrag(toCapture, pointerId)) {
                        break;
                    }
                }
                saveLastMotion(ev);
        ...
        try {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN: {
                    ...
                    if (toCapture == mCapturedView && (mDragState == STATE_SETTLING || mDragState == STATE_DRAGGING)) {
                        tryCaptureViewForDrag(toCapture, pointerId);
                    }
                ...
                case MotionEvent.ACTION_MOVE: {
                    if (mDragState == STATE_DRAGGING) {
                        ...
                        dragTo(-mCapturedView.getScrollX() + idx, -mCapturedView.getScrollY() + idy, idx, idy);
    ...
    private boolean checkTouchSlop(View child, float dx, float dy) {
        ...
        if (checkHorizontal && checkVertical) {
            ...
            return Math.abs(dx) > mTouchSlop && Math.abs(dy)/Math.abs(dx) <= mAngleSlop;
        } else if (checkVertical) {
            return Math.abs(dy) > mTouchSlop && Math.abs(dx)/Math.abs(dy) <= mAngleSlop;
        }
        return false;
    }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        int clampedX = left;
        int clampedY = top;
        final int oldLeft = -mCapturedView.getScrollX();
        final int oldTop = -mCapturedView.getScrollY();
        if (dx != 0) {
