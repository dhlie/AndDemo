...
class ZoomLayoutManager : RecyclerView.LayoutManager() {
    ...
    fun scaleTo(scaleTo: Float): Boolean {
        ...
        for (index in 0 until childCount) {
            ...
            when {
                pivotY >= decoBottom -> {
                    totalVerPadding += getTopDecorationHeight(child) + getBottomDecorationHeight(
                        child
                    )
                }
        ...
        var scaledTop =
            (pivotY - totalVerPadding - (pivotY - top - totalVerPadding) * ratioScale).toInt()

        //(pivotX - scaledLeft - totalHorPadding) / (pivotX - childrenLeft - totalHorPadding) = ratioScale
        var scaledLeft =
            (pivotX - totalHorPadding - (pivotX - childrenLeft - totalHorPadding) * ratioScale).toInt()
        // trim top
     ...
     * 计算滚动条的位置
     *
     * see [com.dhl.base.recyclerview.zoomrecyclerview.ZoomLayoutManager.computeVerticalScrollOffset]
     * see [com.dhl.base.recyclerview.zoomrecyclerview.ZoomLayoutManager.computeVerticalScrollRange]
     * see [com.dhl.base.recyclerview.zoomrecyclerview.ZoomLayoutManager.computeVerticalScrollExtent]
     */
    override fun computeVerticalScrollOffset(state: RecyclerView.State): Int {
        ...
        val laidOutArea =
            orientationHelper.getDecoratedEnd(endChild) - orientationHelper.getDecoratedStart(
                startChild
            )
        ...
        return round(
            itemsBefore * avgSizePerRow + ((orientationHelper.startAfterPadding - orientationHelper.getDecoratedStart(
                startChild
            )))
        ).toInt()
    }
    ...
    private fun fill(
        ...
        while (nextTop < height && nextPosition < itemCount) {
            var view: View? =
                detachedViewCache.get(nextPosition).apply { detachedViewCache.remove(nextPosition) }
            if (view != null) {
                attachView(view)
     ...
     */
    private fun findTopViewPosition(
        left: Int,
        top: Int,
        reLayout: Boolean,
        recycler: RecyclerView.Recycler,
        ...
        if (topView != null && bottomView != null) {
            ...
            if (topViewTop <= 0 && bottomViewBottom >= height && top == topViewTop && firstVisiblePosition == getPosition(
                    topView
                )
            ) {
    ...
    private fun measureChildWithScale(child: View, widthUsed: Int, heightUsed: Int) {
        ...
        val scaledWidth =
            ((width - (paddingLeft + paddingRight + lp.leftMargin + lp.rightMargin + widUsed)) * scale).toInt()
